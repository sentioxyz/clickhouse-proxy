...
func (p *Proxy) handleDropDatabase(ctx context.Context, clientConn net.Conn, id int64, originalSQL, dbName, userAddr string, settings map[string]string) bool {
	...
	if p.coreState == nil {
		log.Errorf("[conn %d] drop_database: core state not configured", id)
		sendExceptionToClient(clientConn, 999, "PROXY_ERROR",
			"DROP DATABASE: proxy has no state mirror")
		return true
	}
	...
	db, ok := p.coreState.GetDatabase(dbName)
	if !ok {
	...
	if !state.IsDatabaseWriter(p.coreState, userAddr, dbName) {
